    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 mb-0">Client Plans</h1>
                <p class="text-muted mb-0">Choose a plan that fits your project needs</p>
            </div>
                    ...
                    <div class="col-lg-4 col-md-6 mb-4">
                        <div class="card h-100 shadow-sm border-0">
                            <div class="card-body text-center">
                                ...
                                @if(auth()->user()->activeClientSubscription() && auth()->user()->activeClientSubscription()->subscription_plan_id === $plan->id)
                                    <button class="btn btn-success w-100" disabled>Current Plan</button>
                                @else
                                    <a href="{{ route('client.show-plan-checkout', $plan->id) }}" class="btn btn-outline-primary w-100">Upgrade to {{ $plan->name }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
